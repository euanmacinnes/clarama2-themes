...
(function(){
  ...
  function loadState(){
    ...
    } catch(e){ isRecording = false; sessionStart = Date.now(); }
  }
  function saveState(){
  ...
  function clear(){ saveEvents([]); flashMsg('Recording cleared'); }
  ...
  function eventsToYaml(){
    ...
    for(const ev of events){
      ...
      if(ev.type === 'click'){
        ...
        actions.push({ type: 'get_by_role', role: 'document', name: '', actions: [{ type: 'press', key: ev.key }]});
  ...
  function injectWidget(){
    ...
    qs('clarama-rec-start').onclick = function(){ try{ if (window.showRecorderCountdown) { window.showRecorderCountdown(3, start); } else { start(); } } catch(e){ start(); } };
    qs('clarama-rec-stop').onclick = stop;
    qs('clarama-rec-export').onclick = function(){ downloadYaml(eventsToYaml()); };
  ...
  function flashMsg(msg){
    try { if (window.flash) { window.flash(msg, 'info'); return; } } catch(e){}
  ...
  <div class="modal fade" id="recorderCountdownModal" tabindex="-1" aria-hidden="true">
    ...
    function showRecorderCountdown(seconds, onProceed){
      try{ ensureRecorderModal(); } catch(e){}
      const modalEl = document.getElementById('recorderCountdownModal');
      if (!modalEl){ onProceed && onProceed(); return; }
      ...
      if (cancelBtn) cancelBtn.onclick = () => { cancelled = true; cleanup(); bsModal.hide(); };
      if (startNow) startNow.onclick = () => { cancelled = false; cleanup(); bsModal.hide(); onProceed && onProceed(); };
      bsModal.show();
      timerId = setInterval(() => {
        remaining -= 1;
        if (remaining <= 0){
          clearInterval(timerId); timerId = null;
          if (!cancelled){ bsModal.hide(); onProceed && onProceed(); }
        }
    ...
    function wireNavbarRecorder(){
      try{
        ...
        if (!btn || !icon) return;
        function isRecOn(){
          try { const s = JSON.parse(localStorage.getItem(STATE_KEY)||'{}'); return !!s.isRecording; } catch(e){ return false; }
        }
        function setState(on){ updateRecordingUI(on); }
        setState(isRecOn());
        btn.addEventListener('click', () => {
          const on = isRecOn();
          if (!on){
            showRecorderCountdown(3, () => {
              if (window.ClaramaRecorder){ window.ClaramaRecorder.start(); }
              else {
            ...
            if (window.ClaramaRecorder){ window.ClaramaRecorder.stop(); }
            else {
      ...
      } catch(e){ console.warn('Navbar recorder wiring failed', e); }
    }
